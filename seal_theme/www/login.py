...
def setup_login(context):
	...
	context.login_background_img = (settings.login_background_img if settings and settings.login_background_img else None)
	context.login_bg_opacity = _percent_to_unit(
		getattr(settings, "login_background_img_opacity", None) if settings else None, 
		default=100
		)

	# background for left panel
	context.login_context_img = (settings.login_context_img if settings and settings.login_context_img else None)
	context.login_context_img_opacity = _percent_to_unit(
		getattr(settings, "login_context_img_opacity", None) if settings else None, 
		default=100
		)

""" Convert 0–100 to 0.0–1.0 safely. """
def _percent_to_unit(pct, default=100):
    try:
        v = int(pct) if pct is not None else default
